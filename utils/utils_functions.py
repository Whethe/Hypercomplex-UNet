...
def mse(y, y_pred):
    if isinstance(y, torch.Tensor):
        return torch.mean((y - y_pred) ** 2).item()  # Return scalar
...
def display_img(x, mask, y, y_pred, score=None):
    ...
    ax3.set_title('GT')
    ax4.imshow(normalize(y_pred), cmap='gray')
...
def check_data(data, name):
    ...
    if np.iscomplexobj(data):
        print(f"{name} (magnitude) - min: {np.min(np.abs(data))}, max: {np.max(np.abs(data))}, mean: {np.mean(np.abs(data))}")
